        ...
        bg-gray-800 shadow-lg rounded-2xl  py-9 mb-4  w-3/4 ml-auto mr-auto ">
          ...
          <form className="px-32" onSubmit={handleSubmit(handleOnSubmitForm)}>
            <div className="mb-4 mt-4">
              ...
              {errors.name?.message && <p className="text-red-500 text-sm text-left mt-3">{errors!.name!.message + ""}</p>}
            </div>
            ...
            {response?.data && (
              <>
                <div className="bg-gray-900 py-3 px-4 mt-8 rounded-2xl w-full mx-auto">

                  <p className="text-left text-gray-100 text-lg whitespace-pre-wrap">{response.data}</p>
                  <button disabled={isSendingtoCommunity} onClick={handleSendtoCommunity} className="disabled:bg-[#9c9dc5] bg-[#6469ff] hover:bg-[#494dc0] text-gray-100 font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mt-8">{isSendingtoCommunity ? "Sending ..." : "Send the results to the Community"}</button>
                </div>
              </>
            )}
          </form>
